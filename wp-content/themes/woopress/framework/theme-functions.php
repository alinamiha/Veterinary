    	...
    	$rating = intval( get_comment_meta( $comment->comment_ID, 'rating', true ) ); ?>
			...
			<li id="comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
				<div class="media">
					...
					<?php comment_text(); ?>
					<?php comment_reply_link(array_merge($args, array('reply_text' => __('Ответить >', ETHEME_DOMAIN),'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
...
if(!function_exists('etheme_custom_comment_form')) {
    function etheme_custom_comment_form($defaults) {
        ...
        $defaults['comment_field'] = '<div class="form-group"><label for="comment" class="control-label">'.__('Comment', ETHEME_DOMAIN).'</label><textarea class="form-control required-field" placeholder="Введите сообщение..." id="comment" name="comment" cols="45" rows="12" aria-required="true"></textarea></div>';
    ...
    function etheme_custom_comment_form_fields() {
        ...
        $fields = array(
            'author' => '<div class="form-group comment-form-author">'.
                            ...
                            '<input id="author" placeholder="Имя" name="author" type="text" required class="form-control' . ($req ? ' required-field' : '') . '" value="' . esc_attr($commenter['comment_author']) . '" size="30" ' . $aria_req . '>'.
                            '<div class="invalid-feedback">Введите имя</div>',
                '</div>',
            'email' => '<div class="form-group comment-form-email">'.
                            '<label for="email" class="control-label">'.__('Email', ETHEME_DOMAIN).' '.($req ? $reqT : '').'</label>'.
                            '<input id="email" placeholder="Email" name="email" type="email" required class="form-control ' . ($req ? ' required-field' : '') . '" value="' . esc_attr($commenter['comment_author_email']) . '" size="30" ' . $aria_req . '>'.
...
if(!function_exists('etheme_create_posts_slider')) {
    function etheme_create_posts_slider($args,$title = false, $more_link = true, $date = false, $excerpt = false, $width = 400, $height = 270, $crop = true, $layout = '', $items = '[[0, 1], [479,2], [619,2], [768,2],  [1200, 3], [1600, 3]]', $el_class = ''){
        ...
        if ( $multislides->have_posts() ) :
            $title_output = '';
            if ($title) {
                $title_output = '<h3 class="title"><span>'.$title.'</span></h3><div style="border: 1px solid #E5E5E5;"></div>';
            }
              echo '<div class="slider-container '.$class.$el_class.'">';
                  ...
                  echo '<div class="carousel-area posts-slider slider-'.$box_id.'">';
                        ...
                        while ($multislides->have_posts()) : $multislides->the_post();
                            ...
                            echo '<a href="'.get_permalink().'" class="link-in-the-dir">';
...
if(!function_exists('etheme_breadcrumbs')) {
    function etheme_breadcrumbs() {
      ...
      $home = __('Главная', ETHEME_DOMAIN); // text for the 'Home' link
      $blogPage = __('Статья', ETHEME_DOMAIN);
      ...
      if (is_front_page()) {
        ...
        if ( is_category() ) {
          ...
          echo $before . 'Статьи по тегу ' . single_tag_title('', false) . $after;
...
if(!function_exists('et_back_to_page')) {
    function et_back_to_page() {
        echo '<a class="back-history" href="javascript: history.go(-1)">'.__('< В категорию',ETHEME_DOMAIN).'</a>';
    }
}
...
if(!function_exists('et_get_related_posts')) {
    function et_get_related_posts($postId = false, $limit = 5){
        ...
        if ($categories) {
            ...
            etheme_create_posts_slider($args, __('Рекомендованые статьи', ETHEME_DOMAIN), false, true, true);
        }
    }
}
